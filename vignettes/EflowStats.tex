...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: package 'xtable' was built under R version 3.1.1}}\end{kframe}
\end{knitrout}
...
First, it is a good practice to clear any existing objects from your working environment. In RStudio, in the upper right quadrant, there is a broom icon that says \enquote{Clear}. Click this and select \enquote{Yes} to remove objects. This allows you to begin work with a clear workspace, negating the possibility of mis-identified variables. 
...
To calculate statistics for USGS streamgage(s), enter gage id(s), start and end dates and a list of the groups of statistics to be calculated. The list in the example contains all possible statistics.
...
Once this information is entered, the user runs the \texttt{}ObservedStatsUSGS\texttt{} function and maps the result to a named data frame.
...
The retrieved data is now present as a dataframe called \texttt{"}statsout\texttt{"} in your R environment. This data frame has one row for each gage and one column for each statistic. An example of the first few columns is given below.

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{verbatim}
   site_no lam1Obs tau2Obs tau3Obs tau4Obs ar1Obs phaseObs
1 02177000  697.04    0.40    0.31    0.15   0.55    -1.15
2 02178400  190.70    0.39    0.36    0.25   0.47    -1.05
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\hlkwd{write.table}\hlstd{(statsout,} \hlkwc{file} \hlstd{= output,} \hlkwc{col.names} \hlstd{=} \hlnum{TRUE}\hlstd{,}
            \hlkwc{row.names} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlkwc{quote} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlkwc{sep} \hlstd{=} \hlstr{"\textbackslash{}t"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}

For daily discharge data not available via NWISWeb, statistics can be calculated by loading data into an R dataframe like the example \texttt{"}dailyData\texttt{"}. 
...
This dataframe has two columns, the first named date, containing dates as characters in the format \texttt{"}YYYY-MM-DD\texttt{"} and the second column, discharge, containing the numeric discharge. This can be easily accomplished from most text file formats using \texttt{"}read.delim\texttt{"} or \texttt{"}read.table\texttt{"}. For example, if the data file to be loaded is saved in dailyData.csv (comma-separated variable) format, with dates and discharges eg:

\begin{table}[ht]
  \begin{threeparttable}[b]
  \caption{dailyData.csv}
  \label{tab:dailyData}
  \begin{tabularx}{150pt}{|X|}
  \hline
\textbf{date,discharge} \\ 
  \hline\hline
  2010-10-01,79 \\ \hline
  2010-10-02,55 \\ \hline
  2010-10-03,52 \\ \hline
...
The user could load this data using \texttt{"}read.table\texttt{"}:

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#load locally stored daily discharge data}
\hlstd{dailyData} \hlkwb{<-} \hlkwd{read.table}\hlstd{(}\hlstr{"dailyData.csv"}\hlstd{,}\hlkwc{sep}\hlstd{=}\hlstr{","}\hlstd{,}
                        \hlkwc{stringsAsFactors}\hlstd{=}\hlnum{FALSE}\hlstd{,}\hlkwc{header}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}

Once the daily discharge data is loaded, the user must enter a drainage area and a site id for each site, as well as the stats string.
...
The generated \texttt{"}statsout\texttt{"} dataframe has the same characteristics as that shown previously. 
...
\includegraphics[width=\maxwidth]{figure/plotMonthlyMeans-1} \caption[Monthly Average Flow at station 02178400]{Monthly Average Flow at station 02178400\label{fig:plotMonthlyMeans}}
\end{figure}


\end{knitrout}

There is one function, \texttt{}CompareStats\texttt{}, that is used to compare the data and calculated statistics for two data sets. It can be used in several ways, three of which are shown here. 

To compare the NWIS observed data for a group of USGS stations with locally stored data (eg modeled data), a list of sites, startDt, endDt, stats string and dataPath are required. The function will then calculate ERSS for each data set, compare the ERSS and directly compare the data sets. These results are then returned in a list (assigned to \texttt{"}DiffStats\texttt{"} in the example). Statistics for the first data set (the NWIS data) are first in the list, then ERSS for the local data, then ERSS comparison, then ERSS goodness of fit and last the direct data goodness of fit.
...
\hlstd{stats}\hlkwb{=}\hlstr{"magnifSeven,magStat,flowStat,durStat,timStat,rateStat,otherStat"}
\hlstd{dataPath}\hlkwb{=}\hlstr{"C:/Users/jlthomps/Documents/R/JData/modeled/"}
\hlstd{DiffStats} \hlkwb{<-} \hlkwd{CompareStats}\hlstd{(stats,}\hlkwc{sites}\hlstd{=sites,}\hlkwc{dataPath}\hlstd{=dataPath,}
                          \hlkwc{startDt}\hlstd{=startDt,}\hlkwc{endDt}\hlstd{=endDt)}
\hlstd{statsLocal} \hlkwb{<-} \hlstd{DiffStats[[}\hlnum{1}\hlstd{]]}
\hlstd{statsNWIS} \hlkwb{<-} \hlstd{DiffStats[[}\hlnum{2}\hlstd{]]}
...
\hlstd{stats}\hlkwb{=}\hlstr{"magnifSeven,magStat,flowStat,durStat,timStat,rateStat,otherStat"}
\hlstd{DiffStats} \hlkwb{<-} \hlkwd{CompareStats}\hlstd{(stats,}\hlkwc{sites}\hlstd{=sites,}\hlkwc{startDt}\hlstd{=startDt,}
                          \hlkwc{endDt}\hlstd{=endDt,}\hlkwc{startDt2}\hlstd{=startDt2,}\hlkwc{endDt2}\hlstd{=endDt2)}
\end{alltt}
\end{kframe}
\end{knitrout}

In order to compare two locally stored data sets, the \texttt{}CompareStats\texttt{} function requires the stats string, dataPath and dataPath2. Start and end date(s) could be provided if desired. The list returned will contain ERSS for both data sets, the ERSS comparison and goodness of fit and the direct data comparison for any overlapping dates. 
...
A typical scientific use case for this package may be the exploration of change in selected hydrologic indices over time as a selected stream undergoes a change in land use, such as increased urbanization. In a developing watershed, for example, a user may wish to calculate fh1 (high flood pulse count), dh15 (high flow pulse duration) and ra6 (positive change of flow) for both pre-development and post-development study periods. This could be accomplished in two ways. The easiest way is to use the provided wrapper functions and then select the desired statistics from the resulting \enquote{statsout} data frame. Alternatively, to calculate only selected indices, an example workflow would be:

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# NWIS-NWIS}
\hlstd{sites} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"02186000"}\hlstd{)}
\hlstd{startDt} \hlkwb{<-} \hlstr{"1990"}
\hlstd{endDt} \hlkwb{<-} \hlstr{"1999"}
\hlstd{startDt2} \hlkwb{<-} \hlstr{"2000"}
\hlstd{endDt2} \hlkwb{<-} \hlstr{"2008"}
\hlstd{data1} \hlkwb{<-} \hlkwd{getDataUSGS}\hlstd{(sites,startDt,endDt)[[}\hlnum{1}\hlstd{]]}
\hlstd{data2} \hlkwb{<-} \hlkwd{getDataUSGS}\hlstd{(sites,startDt2,endDt2)[[}\hlnum{1}\hlstd{]]}
\hlstd{fh1Reference} \hlkwb{<-} \hlkwd{fh1.2}\hlstd{(data1)[[}\hlnum{1}\hlstd{]]}
\hlstd{fh1Urbanized} \hlkwb{<-} \hlkwd{fh1.2}\hlstd{(data2)[[}\hlnum{1}\hlstd{]]}
\hlstd{dh15Reference} \hlkwb{<-} \hlkwd{dh15.16}\hlstd{(data1)[[}\hlnum{1}\hlstd{]]}
\hlstd{dh15Urbanized} \hlkwb{<-} \hlkwd{dh15.16}\hlstd{(data2)[[}\hlnum{1}\hlstd{]]}
\hlstd{ra6Reference} \hlkwb{<-} \hlkwd{ra6}\hlstd{(data1)}
\hlstd{ra6Urbanized} \hlkwb{<-} \hlkwd{ra6}\hlstd{(data2)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
%------------------------------------------------------------ 
To install EflowStats:

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{install.packages}\hlstd{(}\hlstr{"EflowStats"}\hlstd{,}\hlkwc{repos}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{"http://usgs-r.github.com"}\hlstd{,}
                                      \hlstr{"http://cran.us.r-project.org"}\hlstd{),}
                 \hlkwc{type}\hlstd{=}\hlstr{"source"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}

It is a good idea to re-start R after installing the package, especially if installing an updated version. 
